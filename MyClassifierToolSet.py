...
def my_LogRegClassifier(df1,list_predictors,string_target):
	...
	print(test[string_target].value_counts())
	print('\n\n')
	...
	# print('The false positive and True positive rate on test data',(fpr, tpr))
	auc_test = metrics.auc(fpr,tpr)

	print('\n Accuracy Train ------- Accuracy Test------------ AUC Train-------AUC Test--------')
	print(round(accuracy_train,2),round(accuracy_test,2),(auc_train,auc_test))
	...
	cfm=pd.crosstab(label_test,pd.Series(pred_test),rownames=['ACTUAL'],colnames=['PRED'])
	sns.heatmap(cfm,annot=True)
	plt.show()
	print('---------Performance on minority class on test data------------------\n')
